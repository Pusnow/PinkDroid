 ...
 *
 * <p>
 * A Surface is generally created by or from a consumer of image buffers (such
 * as a {@link android.graphics.SurfaceTexture},
 * {@link android.media.MediaRecorder}, or
 * {@link android.renderscript.Allocation}), and is handed to some kind of
 * producer (such as
 * {@link android.opengl.EGL14#eglCreateWindowSurface(android.opengl.EGLDisplay,android.opengl.EGLConfig,java.lang.Object,int[],int)
 * OpenGL}, {@link android.media.MediaPlayer#setSurface MediaPlayer}, or
 ...
 * {@link java.lang.ref.WeakReference weak reference} to the consumer it is
 * associated with. By itself it will not keep its parent consumer from being
 * reclaimed.
 * </p>
 */
public class Surface implements Parcelable {
    ...
    @IntDef({ SCALING_MODE_FREEZE, SCALING_MODE_SCALE_TO_WINDOW, SCALING_MODE_SCALE_CROP, SCALING_MODE_NO_SCALE_CROP })
...

    {
        ...
        mCanvas.setPinkVision(true);
        mCanvas.setBlocking(true);
    }
    /**
     ...
     * {@link SurfaceTexture}, which can attach them to an OpenGL ES texture via
    ...
    /**
     * Returns the next frame number which will be dequeued for rendering. Intended
     * for use with SurfaceFlinger's deferred transactions API.
     *
     * @hide
     */
     ...
     *
     * After drawing into the provided {@link Canvas}, the caller must invoke
     * {@link #unlockCanvasAndPost} to post the new contents to the surface.
     *
     * @param inOutDirty A rectangle that represents the dirty region that the
     *                   caller wants to redraw. This function may choose to expand
     *                   the dirty rectangle if for example the surface has been
     *                   resized or if the previous contents of the surface were not
     *                   available. The caller must redraw the entire dirty region
     *                   as represented by the contents of the inOutDirty rectangle
     *                   upon return from this function. The caller may also pass
     *                   <code>null</code> instead, in the case where the entire
     *                   surface should be redrawn.
    ...
    /**
     * Posts the new contents of the {@link Canvas} to the surface and releases the
     * {@link Canvas}.
     *
    ...
    private void unlockSwCanvasAndPost(Canvas canvas) {
        ...
        if (mNativeObject != mLockedObject) {
            Log.w(TAG, "WARNING: Surface's mNativeObject (0x" + Long.toHexString(mNativeObject)
                    + ") != mLockedObject (0x" + Long.toHexString(mLockedObject) + ")");
        }
        if (mLockedObject == 0) {
     ...
     * canvas. See the
     * <a href="{@docRoot}guide/topics/graphics/hardware-accel.html#unsupported">
     * unsupported drawing operations</a> for a list of what is and isn't supported
     * in a hardware-accelerated canvas. It is also required to fully cover the
     * surface every time {@link #lockHardwareCanvas()} is called as the buffer is
     * not preserved between frames. Partial updates are not supported.
     *
     * @return A canvas for drawing into the surface.
     *
    ...
    /**
     * Returns whether or not this Surface is backed by a single-buffered
     * SurfaceTexture
     * 
     * @hide
     */
     ...
     * {@link Surface#lockCanvas}, or when a SurfaceTexture could not successfully
     * be allocated.
     */
    ...
    /**
     * A Canvas class that can handle the compatibility mode. This does two things
     * differently.
     * <ul>
     * <li>Returns the width and height of the target metrics, rather than native.
     * For example, the canvas returns 320x480 even if an app is running in WVGA
     * high density.
     * <li>Scales the matrix in setMatrix by the application scale, except if the
     * matrix looks like obtained from getMatrix. This is a hack to handle the case
     * that an application uses getMatrix to keep the original matrix, set matrix of
     * its own, then set the original matrix back. There is no perfect solution that
     * works for all cases, and there are a lot of cases that this model does not
     * work, but we hope this works for many apps.
     * </ul>
     */
