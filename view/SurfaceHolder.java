 ...
 * monitor changes to the surface. This interface is typically available through
 * the {@link SurfaceView} class.
 * 
 * <p>
 * When using this interface from a thread other than the one running its
 * {@link SurfaceView}, you will want to carefully read the methods
 * {@link #lockCanvas} and {@link Callback#surfaceCreated
...
public interface SurfaceHolder {
     ...
     * the surface. When used with a {@link SurfaceView}, the Surface being held is
     * only available between calls to {@link #surfaceCreated(SurfaceHolder)} and
    ...
    public interface Callback {
         ...
         * Note that only one thread can ever draw into a {@link Surface}, so you should
         * not draw into the Surface here if your normal rendering will be in another
         * thread.
         * 
         ...
         * returning from this call, you should no longer try to access this surface. If
         * you have a rendering thread that directly accesses the surface, you must
    ...
    public interface Callback2 extends Callback {
         ...
         * redraw is complete, you can ensure that the user will not see your surface in
         * a bad state (at its new size before it has been correctly drawn that way).
         * This will typically be preceeded by a call to {@link #surfaceChanged}.
         *
         * @param holder The SurfaceHolder whose surface has changed.
         */
     ...
     * working with a {@link SurfaceView}, this must be called from the same thread
     ...
     * {@link Callback#surfaceChanged} for changes to the size of the surface. When
     * working with a {@link SurfaceView}, this must be called from the same thread
     * running the SurfaceView's window.
     */
    ...
    /**
     * Set the desired PixelFormat of the surface. The default is OPAQUE. When
     * working with a {@link SurfaceView}, this must be called from the same thread
     * running the SurfaceView's window.
     * 
     ...
     * displayed. The default is false, allowing it to turn off. This is safe to
     * call from any thread.
     * 
     * @param screenOn Set to true to force the screen to stay on, false to allow it
     *                 to turn off.
     */
     ...
     * draw into the surface's bitmap. A null is returned if the surface has not
     * been created or otherwise cannot be edited. You will usually need to
     * implement {@link Callback#surfaceCreated Callback.surfaceCreated} to find out
     * when the Surface is available for use.
     * 
     * <p>
     ...
     * {@link Callback#surfaceDestroyed Callback.surfaceDestroyed}), your calls will
     * be throttled to a slow rate in order to avoid consuming CPU.
     * 
     * <p>
     * If null is not returned, this function internally holds a lock until the
     * corresponding {@link #unlockCanvasAndPost} call, preventing
     * {@link SurfaceView} from creating, destroying, or modifying the surface while
     * it is being drawn. This can be more convenient than accessing the Surface
     * directly, as you do not need to do special synchronization with a drawing
     * thread in {@link Callback#surfaceDestroyed Callback.surfaceDestroyed}.
     * 
     ...
     * outside the dirty rectangle will be preserved by the next call to
     * lockCanvas().
     * 
     ...
     * Rect. This is only safe to call from the thread of {@link SurfaceView}'s
     * window, or while inside of {@link #lockCanvas()}.
     * 
     ...
     * -- for example when using a {@link SurfaceView} the holder's Surface is not
     * created until the view has been attached to the window manager and performed
     * a layout in order to determine the dimensions and screen position of the
     * Surface. You will thus usually need to implement
     * {@link Callback#surfaceCreated Callback.surfaceCreated} to find out when the
     * Surface is available for use.
     * 
     * <p>
     * Note that if you directly access the Surface from another thread, it is
     * critical that you correctly implement {@link Callback#surfaceCreated
     * Callback.surfaceCreated} and {@link Callback#surfaceDestroyed
     * Callback.surfaceDestroyed} to ensure that thread only accesses the Surface
     * while it is valid, and that the Surface does not get destroyed while the
     * thread is using it.
     * 
     * <p>
     * This method is intended to be used by frameworks which often need direct
     * access to the Surface object (usually to pass it to native code).
     * 
